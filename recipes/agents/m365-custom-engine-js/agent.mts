 ...
 */
export function priceTheDeployment(): boolean {
...
async function* fakeStream(kw: ChatRequest) {
 ...
 */
export async function turnScope(
  context: TurnContextLike,
  fn: (stamp: Record<string, string | undefined>) => Promise<void>,
) {
 ...
 */
async function gate(guardrails: Guardrail[], stage: Stage, payload: string, conversationId: string) {
...
const blocked = (decisions: { action: string; guardrail?: string }[]) =>
  decisions.find((d) => d.action === 'block') ?? null;
 ...
 */
function installAudit(path?: string) {
...
async function assemblePrompt(history: ChatMessage[], userText: string) {
  ...
  let squeezedPct: number | undefined;
  if (history.length) {
 ...
 */
export type TurnStateLike = { conversation: Record<string, unknown> };
 ...
 */
export type TurnContextLike = TurnContext;

export class SpendLedger {
  readonly state: TurnStateLike;
  readonly capUsd: Decimal;
  readonly turnCapUsd: Decimal;

  constructor(state: TurnStateLike, capUsd = SESSION_CAP_USD, turnCapUsd = TURN_CAP_USD) {
    this.state = state;
    this.capUsd = capUsd;
...
function turnBudget(
  allowance: Decimal,
  { conversationId, stream = false }: { conversationId: string; stream?: boolean },
  cb: () => Promise<unknown>,
) {
 ...
 */
function preflight(messages: ChatMessage[], allowance: Decimal) {
...
function estimateFor(messages: ChatMessage[]): Decimal | undefined {
  try {
...
function channelDataFor(envelope: Envelope) {
...
export type TurnFacts = {
  ...
  modelCalls: number;
  costUsd?: Decimal;
  sessionSpentUsd?: Decimal;
  sessionCapUsd?: Decimal;
  turnAllowanceUsd?: Decimal;
  estimateUsd?: Decimal;
  traceId?: string;
  assembledMessages: number;
  contextBudgetTokens: number;
  squeezedPct?: number;
  decisions: { guardrail?: string; action: string }[];
  policyCategories: string[];
  auditEntries?: number;
  auditHead?: string;
  /** Where MODEL's rate came from, from `prices.explain()`. A dollar figure with no provenance is
   *  the thing a cost review cannot accept, so the card carries the answer, not just the number. */
  rateFrom?: string;
};

export function newTurnFacts(over: Partial<TurnFacts> = {}): TurnFacts {
  return {
    governance: 'ok',
    inputTokens: 0,
    outputTokens: 0,
...
const usd = (v?: Decimal) => (v === undefined ? '—' : `$${v.toString()}`);
 ...
 */
function narration(f: TurnFacts): string {
  ...
  if (f.governance === 'input_blocked') {
    const names = [...new Set(f.decisions.filter((d) => d.action === 'block').map((d) => d.guardrail))].sort().join(', ');
...
function row(name: string, lib: string, lines: string[]): Record<string, unknown> | null {
    ...
    columns: [
      {
        ...
        items: [
          ...
          { type: 'TextBlock', text: lib, wrap: true, size: 'Small', isSubtle: true, spacing: 'None' },
        ],
      },
      { type: 'Column', width: 'stretch', items: [{ type: 'TextBlock', text, wrap: true, size: 'Small' }] },
    ],
  };
}
 ...
 */
export function governanceCard(f: TurnFacts): Record<string, unknown> {
  ...
  if (f.costUsd !== undefined) budgetLines.push(`this turn **${usd(f.costUsd)}** (decimal.js, never a JS number)`);
  if (f.sessionSpentUsd !== undefined && f.sessionCapUsd !== undefined) {
    budgetLines.push(`session ${usd(f.sessionSpentUsd)} of ${usd(f.sessionCapUsd)}, held in the host's own TurnState`);
  }
  if (f.turnAllowanceUsd !== undefined) budgetLines.push(`this turn's fuse: ${usd(f.turnAllowanceUsd)} (the remainder)`);
  ...
  const ctxLines = f.assembledMessages
    ...
    : [];
  const sqLines = f.squeezedPct === undefined ? [] : [`history compressed **${f.squeezedPct}%** — reversible, not summarised`];

  const gateLines = f.decisions.map((d) => `\`${d.guardrail}\` → **${d.action}**`);
  if (!gateLines.length && f.governance !== 'policy_blocked') gateLines.push('in and out: nothing to act on');

  const auditLines: string[] = [];
  if (f.policyCategories.length) auditLines.push(`data policy stopped: ${f.policyCategories.join(', ')}`);
  if (f.auditEntries !== undefined) auditLines.push(`${f.auditEntries} hash-chained entries`);
  if (f.auditHead) auditLines.push(`head \`${f.auditHead.slice(0, 16)}…\` — \`verify()\` re-walks the file`);
 ...
 */
async function reply(context: TurnContextLike, text: string, envelope: Envelope, facts?: TurnFacts, cards = false) {
...
export type GovernedAgentOptions = {
  auditPath?: string;
  storage?: MemoryStorage;
  sessionCapUsd?: Decimal;
  skipAfterTurnHandler?: boolean;
};
...
export class GovernedAgent {
  ...
  readonly inGate: Guardrail[];
  readonly outGate: Guardrail[];
  readonly sessionCapUsd: Decimal;
  readonly ambient: AmbientProvider;
  readonly audit: AuditLog;
  readonly interceptor: Parameters<typeof addInterceptor>[0];
  ...
  readonly pricedDeployment: boolean;
  ...
  rateProvenance(): string {
    ...
    if (e.registered) return `registered here as \`${BASE_MODEL}\` — your line, outranks every table`;
    if (e.how === 'unpriced') return '**UNPRICED — every USD guard on this turn is a silent no-op**';
  ...
  register() {
    this.app.onActivity(ActivityTypes.Message, async (context, state) => {
      ...
      const turnState = state as unknown as TurnStateLike;
      ...
      await turnScope(context, async () => {
        ...
        const history = (turnState.conversation[HISTORY_PROP] as ChatMessage[]) ?? [];
        const assembly = await assemblePrompt(history, String(inGated.payload));
        const messages = assembly.messages as ChatMessage[];
        ...
        await reply(context, streamed ? '' : String(safeAnswer), envelope, this.turnFacts(facts), this.cards);
      });
    });
  }
   ...
   */
  async create(cap: number, kw: ChatRequest) {
    try {
  ...
  async plainTurn(messages: ChatMessage[], allowance: Decimal) {
    try {
      const resp = await turnBudget(allowance, { conversationId: 'turn' }, () =>
        this.create(MAX_OUTPUT_TOKENS, { model: MODEL, messages }),
      );
      const message = (resp as { choices: { message: { content?: string } }[] }).choices[0].message;
   ...
   */
  async streamedTurn(context: TurnContextLike, messages: ChatMessage[], allowance: Decimal) {
...
export function buildWebApp(agent: GovernedAgent) {
  const app = express();
...
export async function serve(agent: GovernedAgent, port = 3979) {
