 ...
 */
async function reply(context: TurnContextLike, text: string, envelope: Envelope) {
  ...
  register() {
    this.app.onActivity(ActivityTypes.Message, async (context, state) => {
      ...
      await turnScope(context, async () => {
        ...
        const messages = (await assemblePrompt(history, String(inGated.payload))) as ChatMessage[];
