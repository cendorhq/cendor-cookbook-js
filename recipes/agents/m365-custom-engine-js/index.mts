...
type ReplyActivity = {
  text?: string;
  channelData?: { cendor?: Envelope };
  attachments?: { contentType?: string; content?: Record<string, any> }[];
};

class Harness {
  readonly auditPath: string;
  readonly agent: GovernedAgent;
  stub: ChannelStub | null;
  server: Server | null;
  port = 0;

  constructor(
    name: string,
    { sessionCapUsd, skipAfterTurnHandler = false }: { sessionCapUsd?: Decimal; skipAfterTurnHandler?: boolean } = {},
  ) {
  ...
  async turn(text: string, { quiet = 0 }: { quiet?: number } = {}): Promise<ReplyActivity> {
    ...
    return (msgs.at(-1) ?? {}) as ReplyActivity;
  }
}

/** `channelData.cendor` — what the handler attached, off the wire. */
const envelopeOf = (reply: ReplyActivity): Envelope => reply?.channelData?.cendor ?? {};
 ...
 */
const cardOf = (reply: ReplyActivity): Record<string, any> =>
  (reply?.attachments ?? []).find((a) => a.contentType === 'application/vnd.microsoft.card.adaptive')
    ?.content ?? {};
 ...
 */
function renderCard(card: Record<string, any>): string[] {
  const out: string[] = [];
...
let blockedTurn: Envelope & { text: string } = { text: '' };
let redacted: Envelope = {};
try {
...
let capped: Envelope & { text: string } = { text: '' };
let cappedCard: Record<string, any> = {};
...
let preflightTurn: Envelope & { text: string } = { text: '' };
let preflightCard: Record<string, any> = {};
try {
...
let cardOk: Record<string, any> = {};
let cardOkEnv: Envelope = {};
let cardOff: Record<string, any> = {};
try {
...
async function ledgerAfterTwoTurns({ skipAfterTurnHandler }: { skipAfterTurnHandler: boolean }) {
...
assert.ok(blockedTurn.decisions?.some((d: string) => d.includes('prompt_injection')));
assert.ok(redacted.decisions?.some((d: string) => d.includes('email_redact')));
...
assert.ok(cardText.includes(String(cardOkEnv.cost_usd)), "the card's money must be the turn's real cost");
assert.ok(cardText.includes(String(cardOkEnv.trace_id)), 'the card must describe THIS turn');
assert.ok(cardText.includes('rate ') && (cardText.includes('as of') || cardText.includes('outranks every table')));
