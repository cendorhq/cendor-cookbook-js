...
type RecordedActivity = Record<string, unknown>;

export class ChannelStub {
  readonly port: number;
  readonly replies: Map<string, RecordedActivity[]>;
  readonly updates: Map<string, RecordedActivity[]>;
  server: Server | null;
  ...
  async start() {
    ...
    const record = (bucket: Map<string, RecordedActivity[]>) => (req: Request, res: Response) => {
  ...
  async waitFor(
    ...
    { count = 1, timeout = 30_000, quiet = 0 }: { count?: number; timeout?: number; quiet?: number } = {},
  ) {
...
export function makeActivity(
  text: string,
  {
    ...
    serviceUrl,
    channelId = 'emulator',
  }: { conversationId: string; serviceUrl: string; channelId?: string },
) {
  return {
    type: 'message',
    id: randomUUID(),
...
export async function postTurn(endpoint: string, activity: unknown) {
