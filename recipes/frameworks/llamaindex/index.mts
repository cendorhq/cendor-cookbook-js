...
function fakeOpenAI() {
  return {
    chat: {
      completions: {
        create: async (_req: { model: string; messages: unknown[] }) => ({ usage: { prompt_tokens: 2800, completion_tokens: 120 } }),
      },
    },
  };
}
...
const restored = (withHandle?.handle ? withHandle.handle.expand() : null) as string | null;
