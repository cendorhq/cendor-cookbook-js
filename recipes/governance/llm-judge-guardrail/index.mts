...
import { type Guardrail, GuardrailTripped, Verdict, applyAsync, rules } from '@cendor/guardrails';
...
function judgeClient() {
  return instrument({
    chat: {
      completions: {
        create: async (kwargs: { model: string; messages: { role: string; content: string }[] }) => {
...
function makeJudge(client: ReturnType<typeof judgeClient>) {
 ...
 */
async function screenOne(guardrail: Guardrail, prompt: string) {
  try {
 ...
 */
function runSession(guardrail: Guardrail) {
