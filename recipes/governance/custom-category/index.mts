...
const paraMeta = para[0].metadata as { category: string; score: number };
assert.equal(paraMeta.category, 'code_requests');
assert.ok(paraMeta.score > 0.3, 'the recorded similarity score is below the threshold that fired it');
