...
import { type Action, type Stage, Verdict, install, rules, uninstall } from '@cendor/guardrails';
...
type PiiOptions = { policy?: Policy | null; stage?: Stage; action?: Action };

function piiGuardrail({ policy = null, stage = 'input', action = 'redact' }: PiiOptions = {}) {
...
type DecisionPayload = { action: string; stage: string; guardrail: string; reason: string };
...
assert.ok(
  (decisions[0].payload as DecisionPayload).reason.startsWith('pii: '),
...
console.log(`  reason on disk    : ${JSON.stringify((leakDecision.payload as DecisionPayload).reason)}`);
