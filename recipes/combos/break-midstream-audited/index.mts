...
function runawayClient() {
  ...
  const create = async (_req: { model: string; messages: unknown[]; stream: boolean }) => stream;
...
type BudgetEventPayload = { action: string; cap_tokens: number };

const broken = audit.entries.filter(
  (e) => e.type === 'budget_event' && (e.payload as BudgetEventPayload).action === 'broken',
);
...
const cap = (lastBreak.payload as BudgetEventPayload).cap_tokens;
