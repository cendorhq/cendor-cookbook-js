 ...
 */
type LabelledHandle = { technique: string; expand(): unknown };

async function assembleWith(compressor: Parameters<typeof useCompressor>[0], text: string) {
...
const mineHandle = mine.handle as LabelledHandle;
const theirsHandle = theirs.handle as LabelledHandle;

console.log(`raw case log     : ${tokens.count(log, MODEL).toLocaleString('en-US')} tokens, ${log.split('\n').length} lines`);
console.log(`DecisionsOnly    : ${mine.tokensBefore} -> ${mine.tokensAfter} tok  (technique ${mineHandle.technique}, expand() exact: ${mineHandle.expand() === log})`);
console.log(`squeeze (default): ${theirs.tokensBefore} -> ${theirs.tokensAfter} tok  (technique ${theirsHandle.technique}, expand() exact: ${theirsHandle.expand() === log})`);
