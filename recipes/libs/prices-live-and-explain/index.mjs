...
function show(label, model) {
  const e = prices.explain(model);
  console.log(`  ${label.padEnd(11)}: ${e.summary()}`);
  console.log(
    `  ${''.padEnd(11)}  how=${JSON.stringify(e.how)} registered=${e.registered} rowSource=${JSON.stringify(e.rowSource)}`,
  );
...
console.log(
  `  ${'table'.padEnd(11)}: ${prices.sourceName()} (${prices.source()}), ${prices.models().length} rows, snapshotDate=${prices.snapshotDate()}, age=${prices.ageDays()}d`,
);
show('exact', MODEL);
...
console.log(
  `  ${'register'.padEnd(11)}: registerDeployment(${JSON.stringify(DEPLOYMENT)}, { like: ${JSON.stringify(MODEL)} }) -> input=${rates.input} output=${rates.output}`,
);
...
const written = JSON.parse(readFileSync(good, 'utf8'));
console.log(
  `  ${'saved'.padEnd(11)}: prices.json (${Object.keys(written.models).length} rows, _schema=${JSON.stringify(written._schema)}, keeps _provenance=${'_provenance' in written})`,
);
const loaded = await prices.load(good);
console.log(
  `  ${'loaded'.padEnd(11)}: ${loaded} -> source()=${JSON.stringify(prices.source())} sourceName()=${JSON.stringify(prices.sourceName())} snapshotDate=${prices.snapshotDate()}`,
);
show('after load', MODEL);
console.log(
  `  ${''.padEnd(11)}  the registration was re-applied too: explain(${JSON.stringify(DEPLOYMENT)}).registered = ${prices.explain(DEPLOYMENT).registered}`,
);
...
console.log(
  `  ${'default'.padEnd(11)}: refresh(<unreachable>) -> ${quiet}  (table untouched: ${prices.models().length} rows still loaded)`,
);
let thrown = 'nothing';
try {
...
console.log(
  `  ${'required'.padEnd(11)}: refresh(<unreachable>, { required: true }) -> throws ${thrown}`,
);
...
const aged = JSON.parse(readFileSync(good, 'utf8'));
aged._updated = STALE_UPDATED;
writeFileSync(staleFile, JSON.stringify(aged, null, 1), 'utf8');
await prices.load(staleFile);
console.log(
  `  ${'table'.padEnd(11)}: snapshotDate=${prices.snapshotDate()} age=${prices.ageDays()}d isStale(45)=${prices.isStale(45)}`,
);
...
off();
console.log(
  `  ${'two calls'.padEnd(11)}: ${caught.length} StalePriceTableWarning for 2 priced calls (once per process, not per call — a hot loop must not become a log flood)`,
);
console.log(
  `  ${'silence it'.padEnd(11)}: configure({ onStalePrices: 'ignore' }), or move the threshold with stalePricesAfterDays`,
);
...
console.log(
  `  ${'table'.padEnd(11)}: snapshotDate=${prices.snapshotDate()} ageDays()=${prices.ageDays()} isStale(45)=${prices.isStale(45)}`,
);
show('undated', MODEL);
...
if (live) {
  ...
  console.log(
    `  ${'feed'.padEnd(11)}: refresh() -> ${ok}, ${prices.models().length} rows, snapshotDate=${prices.snapshotDate()}, source=${JSON.stringify(prices.sourceName())}`,
  );
  show('gpt-4o', MODEL);
  ...
  if (await prices.refresh(undefined, { source: 'openrouter' })) {
    console.log(
      `  ${'resale'.padEnd(11)}: refresh({ source: 'openrouter' }) -> ${prices.models().length} rows`,
    );
    show('gpt-4o', MODEL);
  }
} else {
  console.log(
    `  ${'offline'.padEnd(11)}: every section above ran on the bundled snapshot, which is GENERATED`,
  );
  console.log(
    `  ${''.padEnd(11)}  from that same feed — which is why rowSource/rowAsof exist here`,
  );
}
