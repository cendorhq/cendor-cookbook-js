...
function fakeClient(streamChunks = 0) {
  return instrument({
    chat: {
      completions: {
        create: async (params: { model: string; messages: unknown[]; stream?: boolean }) => {
          if (params.stream) {
            return {
...
console.log(`                   ${grouped.length} of ${calls.length} calls carry it; the one outside has traceId=${JSON.stringify(outside.traceId)}`);

// ---- seam 2: a per-chunk stream observer ---------------------------------------------------------
const seen: string[] = [];
const meter = (_call: LLMCall, deltaText: string) => {
  seen.push(deltaText);
};
...
if (grouped.length !== 2 || outside.traceId === 'order-8812-refund') throw new Error('trace() did not group');
