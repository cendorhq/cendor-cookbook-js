...
type ResponsesRequest = { model: string; input: string };

function fakeOpenAIResponses(seen: ResponsesRequest[]) {
  return {
    responses: {
      create: async (kwargs: ResponsesRequest) => {
        seen.push(kwargs);
        return {
...
const answer = budget({ usd: 0.4, onExceed: 'block', outputReserve: OUT_TOKENS })(
  async (client: ReturnType<typeof fakeOpenAIResponses>) => {
    for (let i = 0; i < 50; i++) {
