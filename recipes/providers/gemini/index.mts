...
type GenAIRequest = { model: string; contents: string };

function fakeGenAI() {
  const models = {
    generateContent: async (_req: GenAIRequest) => {
      providerCalls.n++;
      return {
        text: 'Refund queued.',
        usageMetadata: {
    ...
    generateContentStream: async (_req: GenAIRequest) => {
      providerCalls.n++;
...
const answer = budget({ usd: 0.5, onExceed: 'block', outputReserve: OUT_TOKENS })(async (client: ReturnType<typeof fakeGenAI>) => {
  for (let i = 0; i < 200; i++) {
